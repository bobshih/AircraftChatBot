...
class WavAudioRecorder {
	private final static int[] sampleRates = {8000, 11025, 16000, 22050, 44100};
	...
	int s_ind = 0;
	int e_ind = 0;
	byte[] buffer_temp = new byte[3 * 3840];
	...
	private AudioRecord.OnRecordPositionUpdateListener updateListener = new AudioRecord.OnRecordPositionUpdateListener() {
		public void onPeriodicNotification(AudioRecord recorder) {
			...
			try {
				energy = 0;
				for (int i = 0; i < buffer.length; i += 2) { // 16bit sample size
					short curSample = getShort(buffer[i], buffer[i + 1]);
					energy += Math.abs((double)curSample);
				}
				energy /= (buffer.length / 2);

				if (init_ind < 2) {
					//Log.e("EXT", "" + threadhold);
					threshold = (init_ind == 0) ? 0:threshold;
					threshold += energy;
					init_ind++;
					if (init_ind == 2) {
						threshold /= 2;
                ...
                if (init_ind < 2) {
                    ...
                    if (flag) {
                        s_ind = (energy > threshold) ? ++s_ind:0;
                        if (s_ind < 3) {
                            System.arraycopy(buffer, 0, buffer_temp, s_ind * buffer.length, buffer.length);
                            if (s_ind == 2) {
                                flag = false;
                                s_ind = 0;
                                randomAccessWriter.write(buffer_temp); // Write buffer to file
                                payloadSize += buffer_temp.length;
                            }
                        }
                    } else {
                        randomAccessWriter.write(buffer); // Write buffer to file
                ...
                */
				Log.e("EXT", "" + buffer.length + " " + threshold + " " + energy);
				//randomAccessWriter.write(buffer); // Write buffer to file
				//payloadSize += buffer.length;
				if (bSamples == 16) {
					for (int i = 0; i < buffer.length; i += 2) { // 16bit sample size
